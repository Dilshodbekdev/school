...
class DirectionalTestsStudentPage extends StatefulWidget {
  const DirectionalTestsStudentPage({super.key});

  @override
  State<DirectionalTestsStudentPage> createState() => _DirectionalTestsStudentPageState();
}

class _DirectionalTestsStudentPageState extends State<DirectionalTestsStudentPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<DirectionalTestStudentBloc, DirectionalTestStudentState>(
        listener: (context,state){
          if(state.isCreatedSimpleTest){
            bloc.getTest(state.createSimpleTestStudentModel?.id??0);
          }
          if(state.isGetSimpleTest){
            context.pushNamed(AppRoutes.directionalTestStudent.name);
          }
          if(state.hasError){
            toastification.show(
                title: Text(
