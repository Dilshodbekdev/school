...
class ScoreTestsStudentPage extends StatefulWidget {
  const ScoreTestsStudentPage({super.key});

  @override
  State<ScoreTestsStudentPage> createState() => _ScoreTestsStudentPageState();
}

class _ScoreTestsStudentPageState extends State<ScoreTestsStudentPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<ScoreTestStudentBloc, ScoreTestStudentState>(
        listener: (context,state){
          if(state.isCreatedSimpleTest){
            bloc.getTest(state.createSimpleTestStudentModel?.id??0);
          }
          if(state.isGetSimpleTest){
            context.pushNamed(AppRoutes.scoreTestStudent.name);
          }
          if(state.hasError){
            toastification.show(
                title: Text(
                    ...
                    LazyLoadScrollView(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) => index == state.completedSimpleTests.length
                          ...
                          onClick: () {
                            context.pushNamed(AppRoutes.completedScoreTestStudent.name,pathParameters: {'id':state.completedSimpleTests[index].id.toString()});
                          },
                        ),
                      ),
                    ):const NoInfoText(),
