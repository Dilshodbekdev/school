...
import 'package:school/src/features/student/test/simple/presentation/widgets/new_simple_test_item.dart';
import 'package:school/src/features/student/test/simple/presentation/widgets/started_simple_test_item.dart';
import 'package:toastification/toastification.dart';

class SimpleTestsStudentPage extends StatefulWidget {
  const SimpleTestsStudentPage({super.key});

  @override
  State<SimpleTestsStudentPage> createState() => _SimpleTestsStudentPageState();
}

class _SimpleTestsStudentPageState extends State<SimpleTestsStudentPage> {
  late final bloc = context.read<SimpleTestStudentBloc>();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<SimpleTestStudentBloc, SimpleTestStudentState>(
        listener: (context,state){
          if(state.isCreatedSimpleTest){
            bloc.getTest(state.createSimpleTestStudentModel?.id??0);
          }
          if(state.isGetSimpleTest){
            context.pushNamed(AppRoutes.simpleTestStudent.name);
          }
          if(state.hasError){
            toastification.show(
                title: Text(
                                      ...
                                      children: [
                                        ...
                                        RichText(
                                          text: TextSpan(
                                            ...
                                            children: <TextSpan>[
                                              TextSpan(text: S.of(context).minutes(((state.newSimpleTests[index].testTime??60)/60).toStringAsFixed(0)),
                                                style: const TextStyle(
                                                fontWeight:FontWeight.w500,
                                                fontSize: 16,
                    ...
                    LazyLoadScrollView(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) => index == state.completedSimpleTests.length
                          ...
                          onClick: () {
                            context.pushNamed(AppRoutes.completedSimpleTestStudent.name,pathParameters: {'id':state.completedSimpleTests[index].id.toString()});
                          },
                        ),
                      ),
                    ):const NoInfoText(),
