...
class CreateScoreTestTeacherPage extends StatefulWidget {
  final int type;
  const CreateScoreTestTeacherPage({super.key, required this.type});

  @override
  State<CreateScoreTestTeacherPage> createState() =>
      _CreateScoreTestTeacherPageState();
}

class _CreateScoreTestTeacherPageState
    extends State<CreateScoreTestTeacherPage> {
  ...
  final form = FormGroup({
    ...
    'ball_descriptions': FormArray([
      FormGroup({
        'description': FormControl<String>(validators: [Validators.required]),
        'id': FormControl<int>(),
        'min': FormControl<int>(validators: [Validators.required]),
        'max': FormControl<int>(validators: [Validators.required]),
      })
    ]),
    'questions': FormArray([
      FormGroup({
  ...
  FormArray get ballDescriptions => form.control('ball_descriptions') as FormArray;
  ...
  FormGroup _getBallDescriptionByIndex(int index) {
    final group = ballDescriptions.controls[index] as FormGroup;
    return group;
  }
  ...
  void initState() {
    ...
    if(widget.type==1){
      ...
      bloc.state.getTestTeacherModel?.ballDescriptions?.asMap().forEach((key1, value1){
        ballDescriptions.add(
            FormGroup({
              'description': FormControl<String>(validators: [Validators.required],value: value1.description),
              'id': FormControl<int>(value: value1.id),
              'min': FormControl<int>(validators: [Validators.required],value: value1.min),
              'max': FormControl<int>(validators: [Validators.required],value: value1.max),
            })
        );
      });
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<ScoreTestTeacherBloc, ScoreTestTeacherState>(
                ...
                children: [
                  ...
                  AppContainer(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          S.of(context).resultsByBall,
                          style: CustomTextStyle.h18SB,
                        ),
                        ReactiveFormArray(
                          formArray: ballDescriptions,
                            builder:(context, array, child){
                              return Column(
                                children: List.generate(ballDescriptions.controls.length, (index){
                                  final ballDescription =_getBallDescriptionByIndex(index);
                                  return Column(
                                    children: [
                                      Row(
                                        children: [
                                          ...
                                          IconButton(onPressed: (){
                                            if(ballDescriptions.controls.length>1){
                                              ballDescriptions.remove(ballDescription);
                                            }else{
                                              snackBar(S.of(context).theremustbeatleastonescoringresult);
                                            }
                                        ...
                                        children: [
                                          Expanded(
                                            child: ReactiveTextField(
                                              formControl: ballDescription.control('min') as FormControl<int>,
                                              decoration: InputDecoration(
                                                filled: true,
                                                isDense: true,
                                                fillColor: AppColors.bgColor,
                                          ...
                                          Expanded(
                                            child: ReactiveTextField(
                                              formControl: ballDescription.control('max') as FormControl<int>,
                                              decoration: InputDecoration(
                                                filled: true,
                                                isDense: true,
                                                fillColor: AppColors.bgColor,
                                      ...
                                      const Gap(16),
                                      ReactiveTextField(
                                        formControl: ballDescription.control('description') as FormControl<String>,
                                        decoration: InputDecoration(
                                          filled: true,
                                          isDense: true,
                                          fillColor: AppColors.bgColor,
                  ...
                  ReactiveFormConsumer(builder: (context, form, child){
                    return AppElevatedButton(
                      ...
                      onClick: (){
                        if(form.valid){
                          List<QuestionScore> questionsModel=[];
                          questions.controls.asMap().forEach((key1, value1){
                            ...
                            final List<AnswerScore> answersModel = [];
                          ...
                          List<BallDescriptionScore> ballDescriptionsModel=[];
                          ballDescriptions.controls.asMap().forEach((key, value){
                            final ballDescription = value as FormGroup;
                            final description = ballDescription.control('description') as FormControl<String>;
                            final id = ballDescription.control('id') as FormControl<int>;
                            final max = ballDescription.control('max') as FormControl<int>;
                            final min = ballDescription.control('min') as FormControl<int>;

                            ballDescriptionsModel.add(
                          ...
                          CreateScoreTestTeacherBody body = CreateScoreTestTeacherBody(
                            name: name.value,
                            ...
                            ballDescriptions: ballDescriptionsModel,
                          );
